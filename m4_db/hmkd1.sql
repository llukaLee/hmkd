...
INSERT INTO MEMBER VALUES('200901','111','kevin','m',25,'2000-01-01','010-1234-5678',(DATE '2000-12-31'));
SELECT * FROM MEMBER;
TRUNCATE TABLE MEMBER;
DESC MEMBER;
ALTER TABLE MEMBER ADD(HOBBY VARCHAR2(10));
ALTER TABLE MEMBER MODIFY(HOBBY VARCHAR2(20));
ALTER TABLE MEMBER DROP COLUMN HOBBY;
ALTER TABLE MEMBER ADD CONSTRAINT MEMBER_PK PRIMARY KEY(ID);
ALTER TABLE MEMBER DROP CONSTRAINT MEMBER_PK;


--과제12_0509. md에 있는 테이블들을 이용해서 hmkd 서점의 데이터 테이블을 생성하고 주요 특징을 요약해서 출력하세요.
--(테이블은 5개 이상)

--Q. MEMBER1 테이블을 생성(모든 데이터 타입 적용)하고 2개의 데이터 입력 및 칼럼 추가, 칼럼 타입 변경, 캄럼 삭제,
--제약조건 추가, 삭제한 후 출력하세요.



-------------------------------------------------------------------------------------------------
ALTER TABLE MEMBER1 MODIFY            -- 칼럼변경
ALTER TABLE MEMBER1 DROP COLUMN       -- 칼럼삭제
ALTER TABLE MEMBER1 ADD               -- 칼럼추가
ALTER TABLE MEMBER1 ADD CONSTRAINT    -- 제약조건 추가
ALTER TABLE MEMBER1 DROP CONSTRAINT   -- 제약조건 삭제
ALTER TRUNCATE TABLE MEMBER1;                -- 테이블내용삭제
---------------------------------------------------------------------------------------------------
...
select EMPLOYEE_ID, MANDGER_ID from employees where manager_id between 101 and 105;
...
--length()
select job_id, length(job_id) from emploees;

--instr(문자열, 찾을 문자, 찾을 위치, 찾은 문자 중 몇 번 째)
select instr('Hellow World', 'o', 3, 2) from dual;

--substr(문자열, 시작위치, 개수)
select instr('Hellow World', 'o', 3, 3) from dual;
select instr('Hellow World', 'o', -3, 3) from dual;

--lpad() , rpad() 왼쪽, 오른쪽 정렬 후 문자를 채운다.
select lpad('Hellow World', 20, '#') from dual;
select rpad('Hellow World', 20, '#') from dual;
...
--case() : decode() 함수와 동일하나 decode() 함순느 조건이 동일한 경우만 가능하지만 case()함수는 다양한 비교연산자로 조건을 제시
select last_name, department_id,
...
insert into member values  ('200903','113','김연아','1',20,'2001-12-30','010-1234-5678',sysdate);
...
update member set name='추신수' where id = '200904';
...
insert into test5 values('111','dick',75);
...
insert into test6 values('111',75,20);
...
--과제4_0510. employees, jobs, departments 세개의 테이블을 연결해서 employee_id, job_title, department_name을 출력하세요.
select job_id, employee_id, last_name from employees;
select * from jobs;
...
--과제5_0510. HR 테이블들을 분석해서 hr에 포함되는 테이블들을 분석해서 인사이트를 얻을 수 있는 결과물을 5개 이상 출력하세요.
--(예 : 부서별 평균 SALARY 순위)-주경형님숙제참조
...
-- rank 1위가 73번째
select rank(1) within group(order by commission_pct desc) from employees;
...
-- *주의*     select 뒤에는 그룹으로 묶을 수 있는 컴럼만 올 수 있다.

--Q. 평균연봉이 5000이상인 부서들에 대해서 job_id 별로 연봉합계 연봉평균 최고연봉 최정연봉 출력
--having
...
--Q. 평균연봉이 5000이상인 부서들에 대해서 job_id 별로 연봉합계 연봉평균 부서인원 출력
--(연봉평균 기준 내림차순 정렬)
select job_id 업무, avg(salary) 연봉평균, count(employee_id) 부서인원
from employees
group by job_id
...
select last_name, salary, job_id, rank() over(partition by job_id order by salary desc) 연봉순위 from employees order by job_id;

--GROUP BY: GROUP BY를 사용하면 각 그룹의 대표값 또는 집계값을 가져올 수 있다. 
--결과 집합은 그룹화된 열과 집계된 열로 구성
--PARTITION BY: PARTITION BY를 사용하면 결과 집합의 각 행에 대해 순위 또는 분석 함수를 
--계산할 수 있다. 결과 집합은 원래의 테이블과 동일한 열들로 구성되며, 
--추가된 열에는 순위 또는 분석 함수의 결과가 포함된다.
--GROUP BY: GROUP BY는 집계 함수 (예: SUM, COUNT, AVG 등)와 함께 사용되어 그룹 단위로 함수를 계산.
--PARTITION BY: PARTITION BY는 분석 함수 (예: RANK, ROW_NUMBER, LEAD, LAG 등)와 함께 
--사용되어 파티션 단위로 함수를 계산

--과제3_0511. employees 테이블에서 employee_id와 salary만 추출해서 employee_salary 테이블을 생성하세요.
drop table employes_salary;
create table employees_salary
as select employees_id, salary
from employees;
...
--과제6_0511. employees_salary 테이블의 employee_id에 기본키를 적용하고 CONSTRAINT_NAME을 ES_PK로 지정 후 출력하세요.
alter table employees_salary add constrate es_pk primary key(employee_id);

--과제7_0511. employees_salary 테이블의 employee_id에서 CONSTRAINT_NAME을 삭제후 삭제 여부를 확인하세요.
alter table employees_salary drop constrate es_pk;
...
GRANT CREATE VIEW, CREATE SYNONYM TO c##hmdk1;
GRANT UNLIMITED TABLESPACE TO c##hmkd1;
ALTER USER c##hmkd1 ACCOUNT UNLOCK;
--사용자와 해당 사용자에게 부여된 모든 객체와 권한을 함께 삭제하도록 "CASCADE" 옵션을 사용하여 사용자를 삭제합니다.
--이는 해당 사용자를 완전히 삭제하려는 경우에 유용합니다.
--"c##hmkd1"이라는 사용자를 생성합니다."IDENTIFIED BY" 키워드 다음의 "hmkd1"은 해당 사용자의 암호를 지정
--"DEFULT TABLESPACE" 키워드 다음의 "users"는 해당 사용자가 기본적으로 사용할 테이블 스페이스를 지정
...
--"RESOURCE" 권한은 해당 사용자가 테이블, 기퀀스, 프로시저 등을 만들 수 있는 권한을 부여
...
--"c##hmkd1" 사용자의 계정 잠금을 해제. 계정 잠금은 일정 시간 동안 로그인 실패가 발생하면 자동으로 발생하는 보안 기능.
--계정 잠금을 해제하여 해당 사용자가 로기인할 수 있도록 합니다.
...
--트랜잭션은 의미적으로 분할할 수 ㅇ벗는 최소의 단위이다. 그렇기 때문에 전부 적용하거나 전부 취소한다.
--즉, TRANSCTION 은 ALL OR NOTHING의 개념인 것이다.
